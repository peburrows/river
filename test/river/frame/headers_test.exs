defmodule River.Frame.HeadersTest do
  ...
  test "we can decode a frame from a padded payload", %{headers: headers, ctx: ctx, payload: payload} do
    ...
    } = Headers.decode(%Frame{length: (4+byte_size(payload)), flags: %{padded: true}}, <<3::8, payload::binary, "pad">>, ctx)
  end
  ...
  test "stream dependency is propery extracted", %{headers: headers, ctx: ctx, payload: payload} do
    ...
    } = Headers.decode(%Frame{length: byte_size(payload), flags: %{priority: true}}, payload, ctx)
  end
  ...
  test "a payload with a priority and padding is properly decoded", %{headers: headers, ctx: ctx, payload: payload} do
    ...
    } = Headers.decode(%Frame{length: byte_size(payload), flags: %{padded: true, priority: true}}, payload, ctx)
  end
end
